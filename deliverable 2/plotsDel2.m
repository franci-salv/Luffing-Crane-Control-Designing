...
simOutlinear = sim("TimeTrajLinear.slx");
qlin_data = simOutlinear.q;
xlin_data= qlin_data(:,1);
thetalin_data = qlin_data(:,2);

time_lin = simOutlinear.tout;
...
plot(time_lin, xlin_data, '--', 'LineWidth', 1.5);

xlim([0 110]);
ylim([11.42 13.08]);

title('Hoist Position x(t): Simulation vs Simulation Linear');
xlabel('Time (s)');
...
plot(time_lin, thetalin_data, '--', 'LineWidth', 1.5);

xlim([0 110]);
ylim([0.95 2.05]);

title('Chain Angle \theta(t): Simulation vs Linear Simulation');
xlabel('Time (s)');
